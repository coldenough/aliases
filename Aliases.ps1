    ...
    [ System.Management.Automation.PSParser ]::Tokenize($line,[ref]$null) | % {
      if ( $_.Type -eq "Command" ) {
        ...
        if ($cmd.CommandType -eq "Alias") {
          $line = $line.Remove( $_.StartColumn -1, $_.Length ).
              Insert( $_.StartColumn -1, $cmd.Definition )
        }
      }
    }
    $line
...
function alias {
  ...
  $alias,$cmd = [string]::join(" ",$args).split("=",2) | % {
    $_.trim()
  }
  ...
  New-Item -Path function: -Name "Global:Alias$Alias" `
      -Options "AllScope" -Value @"
Invoke-Expression '$cmd `$args'
###ALIAS###
"@

  Set-Alias -Name $Alias -Value "Alias$Alias" `
            -Description "A UNIX-style alias using functions" `
            -Option "AllScope" `
            -scope Global `
            -passThru
}
...
function unalias([string]$Alias,[switch]$Force){ 
  if ( (Get-Alias $Alias).Description `
      -eq "A UNIX-style alias using functions" ) {
